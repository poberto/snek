...
struct Snake {
  uint8_t headX, headY;
  uint8_t length;
  uint8_t tailX[MAX_LENGTH];
  uint8_t tailY[MAX_LENGTH];
  uint8_t direction;
...
void displayGameOver() {
  ...
  lc.setChar(0, 0, 'G', true);
  lc.setChar(0, 8, 'A', true);
  lc.setChar(0, 16, 'M', true);
  lc.setChar(0, 24, 'E', true);
  delay(1000);
  lc.clearDisplay(0);
  delay(200);
  lc.setChar(0, 0, 'O', true);
  lc.setChar(0, 8, 'V', true);
  lc.setChar(0, 16, 'E', true);
  lc.setChar(0, 24, 'R', true);
  delay(1000);
  lc.clearDisplay(0);
  delay(200);

  // Wait for input to restart the game
...
void initializeGame() {
  ...
  snake1.headY = ROWS / 2;
  snake1.length = 3;
  snake1.direction = 1; // Right
  snake1.prevDirection = 1;

  // Initialize Snake 1 tail
  for (int i = 0; i < snake1.length; i++) {
    snake1.tailX[i] = snake1.headX - i - 1;
    snake1.tailY[i] = snake1.headY;
  }

//   Initialize Snake 2
  snake2.headX = (3 * COLS) / 4;
  snake2.headY = ROWS / 2;
  snake2.length = 3;
  snake2.direction = 0; // Left
  snake2.prevDirection = 0;

  // Initialize Snake 2 tail
  for (int i = 0; i < snake2.length; i++) {
    snake2.tailX[i] = snake2.headX + i + 1;
    snake2.tailY[i] = snake2.headY;
  }
